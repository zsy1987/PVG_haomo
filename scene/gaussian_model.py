...
class GaussianModel:
    ...
    def load_ply(self, path):
        ...
        time = (np.random.rand(self.get_xyz.shape[0], 1) * 1.2 - 0.1) * (
                self.time_duration[1] - self.time_duration[0]) + self.time_duration[0]
    

   
        random_times = (torch.rand(0, 1, device="cuda") * 1.2 - 0.1) * (
                self.time_duration[1] - self.time_duration[0]) + self.time_duration[0]
        ...
        scales_t = torch.full((self.get_xyz.shape[0], 1), 9999999999999999999999., device="cuda")
        velocity = torch.zeros(self.get_xyz.shape[0], 3, device="cuda")  # 全部初始化为0
        velocity[:, 1] = 1.0  
